...
public class OpportunisticContainerAllocatorAMService
  ...
  private synchronized List<RemoteNode> getLeastLoadedNodes() {
    ...
    if ((currTime - lastCacheUpdateTime > cacheRefreshInterval)
        || cachedNodes == null) {
      cachedNodes = convertToRemoteNodes(this.nodeMonitor.selectLeastLoadedNodes(this.k));
  ...
  private RemoteNode convertToRemoteNode(NodeId nodeId) {
    AbstractYarnScheduler scheduler = (AbstractYarnScheduler) rmContext.getScheduler();
    SchedulerNode node = scheduler.getNode(nodeId);
